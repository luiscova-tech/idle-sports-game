...
- **Unlock-threshold logic chosen:** a locked tier unlocks once the immediately preceding tier's `cumulativeRevenue` (lifetime Revenue earned from that specific tier) reaches that tier's configured `unlockThreshold`, re-checked (`applyTierUnlocks`) every time any tier's match completes.
...
- `tickTier()` in the store now adds `perTickRevenue` to both `currencies.revenue` and that tier's `cumulativeRevenue` on **every** tick, not only at completion. On the completion tick, the unchanged completion-bonus formula is added on top (`totalEarned = perTickRevenue + completionBonus`), never replacing it.
- Because `cumulativeRevenue` can now cross an unlock threshold mid-match, `applyTierUnlocks` runs after every tick's state update (both branches), not only after completion.
...
- **New unlock thresholds and why:** `local-tournament` 600→450, `regional-championship` 3000→2250, `national-league` 15000→11250 (`local-game`'s is unused). Chosen so Local Tournament's threshold sits comfortably (not at a razor's edge) after Local Game's first-purchase point: even in a fast "buy every upgrade the instant affordable" playthrough, 450 is crossed at ≈61s of play — roughly 2.4x the 25s first-purchase mark, and ≈113s in a no-upgrade baseline (≈4.5x) — giving genuine breathing room to make at least one (in practice, both) Local Game purchases before a new tier appears. The three thresholds keep the original 450→2250→11250 = clean 5x/5x growth ladder, preserving the pre-existing design intent that thresholds compound faster than the (unchanged, ~4x/3x/2.9x) multiplier growth per tier.
...
- Playtested end-to-end: first Revenue is now visible after exactly one click (previously required a full 90-tick match); the first affordable purchase (Improve Training) now lands at click/second 25 (previously took multiple full matches, hundreds of clicks); Local Tournament unlocked only after both Local Game purchases were already affordable.
