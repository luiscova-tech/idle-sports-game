...
- **Trigger:** `resetForLegacy()` (`src/store/useGameStore.ts`) is a no-op unless the ladder's final tier (currently World Championship — checked by index, `tiers[tiers.length - 1].unlocked`, not by id, so it stays correct if the ladder grows) is unlocked. The UI (`LegacyPanel.tsx`) shows a locked explainer until then, matching the venture-tier lock-card convention rather than hiding the system outright.
